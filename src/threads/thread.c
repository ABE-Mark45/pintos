...
void threads_wakeup_blocked(int64_t ticks)
{
  ...
  while (!list_empty(&blocked_threads) && (it = list_entry(list_front(&blocked_threads), struct thread, elem)) && it->wake_up_after_tick <= ticks)
  {
    list_remove(&it->elem);
    thread_unblock(it);
...
{
  enum intr_level old_level = intr_disable();
  if (!list_empty(&ready_list) && is_higher_priority_first(list_entry(list_front(&ready_list), struct thread, elem), thread_current()))
    thread_yield();
  intr_set_level(old_level);
}
